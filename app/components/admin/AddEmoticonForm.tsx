...
import { addEmoticons } from '@/app/actions'

export default function AddEmoticonForm({ packId }: { packId: string }) {
    const bindAction = addEmoticons.bind(null, packId)
    ...
    return (
        <form action={action} className="bg-gray-50 p-4 rounded flex gap-2 items-center">
            ...
            <input type="file" name="images" accept="image/*" multiple required className="text-sm" />
            <button
                disabled={isPending}
